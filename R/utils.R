...
#' @param x dbData object to convert to a view \code{(required)}
#' @param name view name to assign within database. auto-generated if none provided \code{(required)}
#' @param temporary if TRUE, the view will be removed after the session ends
#' @concept dbData
#' @export
setMethod(
  'to_view',
  ...
  function(x, name, temporary = TRUE, ...) {
    ...
    sql_view <- glue::glue("CREATE TEMPORARY VIEW '{name}' AS {sql}")
...
#' Convert lazy table to named view
#' @param x `[tbl_duckdb_connection]` object to convert to a view \code{(required)}
#' @param name view name to assign within database. auto-generated if none provided \code{(required)}
#' @param temporary if TRUE, the view will be removed after the session ends
#' @concept dbData
#' @export
setMethod(
  ...
  signature(x = "missing"),
  function(x, name, temporary = TRUE, ...) {
    stop("'x' argument is required for to_view")
  }
)
